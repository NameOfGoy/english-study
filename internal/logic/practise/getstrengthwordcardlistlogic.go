...
type GetStrengthWordCardListLogic struct {
	logx.Logger
	ctx    context.Context
...
func NewGetStrengthWordCardListLogic(ctx context.Context, svcCtx *svc.ServiceContext, ui *utils.UserInfo) *GetStrengthWordCardListLogic {
	return &GetStrengthWordCardListLogic{
...
func (l *GetStrengthWordCardListLogic) GetStrengthWordCardList(req *types.GetWordCardListReq) (resp *types.GetWordCardListResp, err error) {
	...
	if !req.Random {
		...
		if err != nil {
			return nil, errors.ErrorDatabaseQueryError("查询强化中的单词失败").WithCause(err)
		}
	} else {
	...
	for _, v := range wss {
		...
		if err != nil {
			return nil, errors.ErrorRequestParamError("查询强化中的单词失败").WithCause(err)
		}
